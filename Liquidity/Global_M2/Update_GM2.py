...
def UpdateData(M2List:pd.DataFrame,M2Path:str,FXPath:str):
    ...
    for country in Datas.keys():       #Step 1b: Save the M2 & FX Datas to disk. 
        ...
        M2_Data.to_excel(grampa+fdel+"User_Data"+fdel+"GM2_Data"+fdel+"M2_Data"+fdel+country+"_M2.xlsx")
        try:
            FX_Data = pd.DataFrame(CuntryData[1])
            FX_Data.to_excel(grampa+fdel+"User_Data"+fdel+"GM2_Data"+fdel+"FX_Data"+fdel+country+"_FX.xlsx")
        except:
            continue
...
if __name__ == "__main__":
    ...
    top8path = wd+fdel+'UpdateM2Infos'+fdel+'M2Info_Top8.xlsx'

    # show an "Open" dialog box and return the path to the selected file
    M2Path = (grampa+fdel+'User_Data'+fdel+"GM2_Data"+fdel+'M2_Data'); 
    FXPath = (grampa+fdel+'User_Data'+fdel+"GM2_Data"+fdel+'FX_Data') ###Change these if changing the folder structure within "Global_M2" folder.
    ...
    top8com = CombineDatasSimp(top8,top8Sum,M2Path,FXPath) ##Step #3 multiply M2 & FX datas. 

    for country in Combos.keys():
        data = pd.DataFrame(Combos[country])
        data.to_excel(grampa+fdel+"User_Data"+fdel+"GM2_Data"+fdel+"FinalData"+fdel+country+".xlsx")

    FinDataPath = grampa+fdel+"User_Data"+fdel+"GM2_Data"+fdel+"FinalData"+fdel
    ...
    top8fin = MakeMasterM2DF_2(top8Sum,FinDataPath) #Step #4: put all the data in a big master DF. 
    top8fin.to_excel(SaveDataTo+'Top8_M2_USD.xlsx')
    ...
    savePath = grampa+fdel+"User_Data"+fdel+'SavedData'+fdel
    SeriesInfo = pd.Series({'units':'US Dollars','units_short': 'USD','title':'Global M2 '+des,'id':des+'GM2',"Source":"tv"},name='SeriesInfo')
