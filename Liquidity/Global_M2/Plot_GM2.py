...
class YoY_forecast(object):
    ...
    def save_em(self, savePath: str = grampa + fdel + 'User_Data' + fdel + 'SavedData'):
        for forecast in self.forecasted.keys():
...
if __name__ == '__main__':
    ...
    dataPath = grampa + fdel + 'User_Data' + fdel + "GM2_Data" + fdel + 'FinalData'    
...
#################################### ACTIVE CODE BELOW, FUNCTIONS ABOVE. #####################################################      
    ...
    FullDF.set_index(index,inplace=True); FullDF.drop('Date',axis=1,inplace=True)    ######## MatPlotlib functions ########################################################################################################################

    DataComp = DataComp[0:5]
    PlotM2Data(FullDF,DataComp)
    Plot_GlobalM2(FullDF['Global M2 (USD, ffill)'], FullDF)

    series = pd.read_excel(grampa+fdel+'User_Data'+fdel+'SavedData'+fdel+'Top50GM2.xlsx', sheet_name='Closing_Price', index_col=0)
