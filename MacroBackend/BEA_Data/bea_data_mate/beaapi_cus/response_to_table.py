...
# from beaapi_cus.get_row_data_value import get_row_data_value
...
def response_to_table(bea_payload: Union[list, pd.DataFrame, HTTPResponse],
    ...
    ----------
    bea_payload :
        A 'HTTPResponse' returned from beaapi_cus.api_response(...) call to
        BEA API, an object of class 'list' returned from beaapi_cus.response_to_dict(...),
        or an object of class 'pd.DataFrame' returned from
        beaapi_cus.response_to_table(...).
    repull :
    ...
    -------
    pd.DataFrame
        An object of class 'pd.DataFrame' containing data from beaapi_cus.api_respons(...)
    ...
    >>>     'ResultFormat': 'json'
    >>> }
    >>> bea_payload = beaapi_cus.api_respons(beaspecs)
    """
