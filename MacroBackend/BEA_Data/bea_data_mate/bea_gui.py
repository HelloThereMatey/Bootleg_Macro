...
from MacroBackend import Utilities, charting_plotly ## Don't worry if your IDE shows that this module can't be found, it should stil work. 
...
plt.rcParams.update({
    'figure.dpi': 150, 'figure.autolayout': True,
    'savefig.dpi': 150, 'figure.figsize': [10, 6]})
...
def bea_gui():
    ...
    plotly = ctk.BooleanVar(master=root,value=False,name='Use_plotly')
    ...
    savePath = ancestor+fdel+'User_Data'+fdel+'BEA'+fdel+"bea_tables"; export = ancestor+fdel+'User_Data'+fdel+'BEA'
    ...
    plot_button = ctk.CTkButton(bottom2, text="Preview data", text_color='yellow', font=('Arial',14,'bold'),command=plotPreview,border_width=1, width=140, height=30)
    plot_button.grid(column=1,row=0, sticky = 'e', pady=10)
    plotly_check = ctk.CTkCheckBox(bottom2,text='Use Plotly',font=('Arial', 12), variable=plotly, width=50, height=20); plotly_check.grid(column=1,row=0,sticky='w', padx = 10, pady=10)
    ...
    CustomIndex.grid(column=2,row=0, padx = 10, pady=10)
