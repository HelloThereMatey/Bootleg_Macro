...
def get_data(userid: str, datasetname: str, do_checks: bool = False,
    ...
    **kwargs:
    ...
    --------
    >>> import beaapiloc
    >>> beaapiloc.get_dataset('yourAPIkey', 'NIPA', TableName='T20305', Frequency='Q',
    >>>                    Year='X')
    """
    from beaapiloc import api_request, BEAAPIPkgException
...
def to_wide_vars_in_cols(bea_tbl):
    ...
    --------
    >>> import beaapiloc
    >>> tbl = beaapiloc.get_dataset(...)
    >>> wide_tbl = beaapiloc.to_wide_vars_in_cols(tbl)
    """
...
def to_wide_vars_in_rows(bea_tbl):
    ...
    >>> import beaapiloc
    >>> tbl = beaapiloc.get_dataset(...)
    >>> wide_tbl = beaapiloc.to_wide_vars_in_rows(tbl)
    """
    if 'wide_format' in bea_tbl.attrs:
        raise Exception("bea_tbl must be in long-format (e.g., from the API).")
