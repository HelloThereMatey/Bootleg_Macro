...
print('Screen display width x height (pixels): '+str(sw)+' x '+str(sh))
...
Title = Label(top_frame,text='Glassnode Studios data downloader',font=('Arial',16,'bold')); Title.grid(column=1,row=0,padx=60,pady=5,sticky='w',columnspan=2)
btn=Button(top_frame, text="Load GN metrics list",fg='blue',command=LoadPathBtn,font=('Arial',14)); btn.grid(column=0,row=0,sticky='w',padx=10,pady=5)
upd=Button(top_frame, text="Update GN metrics list",fg='fuchsia',command=UpdateMetricList,font=('Arial',14)); upd.grid(column=3,row=0,padx=10,pady=5)
...
update_button = Button(top_frame, text="Search metrics", command=SearchBtn,font=('Arial',14,'bold'),border=2); update_button.grid(column=1,row=2,padx=15,pady=5,sticky='w')
note = Label(top_frame,text='*Double click endpoint path to select metric.',font=('Arial',12)); note.grid(column=2,row=2,padx=15,pady=5)
...
getMetric = Button(mid_left, text="Get data for selected metric", command=getGNData, font=('Arial',11,'bold'))
getMetric.grid(column=0,row=2,sticky='ne',padx=1,pady=1)
note3 = Label(mid_left,text='Metrics/endpoints matching search criteria:',font=('Arial',10)); note3.grid(column=0,row=0,sticky='nw',padx=1,pady=1)
...
note4 = Label(mid_right,text='Tier',font=('Arial',11,'bold')); note4.grid(column=0,row=0,sticky='nw',padx=15,pady=50)
drop2 = OptionMenu(mid_right,assChoice,value='Assets'); drop2.grid(column=1,row=0,padx=10,pady=15)
note5 = Label(mid_right,text='Assets',font=('Arial',11,'bold')); note5.grid(column=1,row=0,sticky='n',padx=10,pady=50)
drop3 = OptionMenu(mid_right,currChoice,value='Currencies'); drop3.grid(column=2,row=0,padx=10,pady=20)
note5 = Label(mid_right,text='Currencies',font=('Arial',11,'bold')); note5.grid(column=2,row=0,sticky='n',padx=5,pady=50)
drop4 = OptionMenu(mid_right,resChoice,'Resolutions'); drop4.grid(column=1,row=1,padx=10,pady=20)
note6 = Label(mid_right,text='Resolutions',font=('Arial',11,'bold')); note6.grid(column=1,row=1,sticky='n',padx=5,pady=30)
drop5 = OptionMenu(mid_right,formChoice,'Formats'); drop5.grid(column=2,row=1,padx=10,pady=20)
note7 = Label(mid_right,text='Formats',font=('Arial',11,'bold')); note7.grid(column=2,row=1,sticky='n',padx=5,pady=30)

text_entry2 = Entry(mid_right2,textvariable=StartDate); text_entry2.grid(column=0,row=0,padx=20,pady=45)
note8 = Label(mid_right2,text='Starting Date (YYYY-MM-DD), blank = max',font=('Arial',10)) ; note8.grid(column=0,row=0,sticky='n',padx=20,pady=20)
text_entry3 = Entry(mid_right2,textvariable=EndDate); text_entry3.grid(column=0,row=1,padx=20,pady=30)
note9 = Label(mid_right2,text='End Date (YYYY-MM-DD), blank = NOW',font=('Arial',10)); note9.grid(column=0,row=1,sticky='n',padx=20)
# BTC_too = Checkbutton(root)
...
plot_button = Button(mid_right2, text="Preview metric",font=('Arial',11,'bold'), command=plotPreview); plot_button.grid(column=0,row=2,sticky='e',padx=10,pady=15)
note2 = Label(mid_right2,text='*Y-scaling of\npreview chart.',font=('Arial',11)); note2.grid(column=0,row=2,sticky='nw',padx=25,pady=15)
...
SetSavePath=Button(bot_frame, text="Set save path",fg='green',command=SetSavingPath); SetSavePath.pack(side='left',padx=15,pady=10)
#SetSavePath.grid(row=1,sticky='w',padx=30,pady=5)
SaveBtn=Button(bot_frame, text="Save data",fg='red',command=SaveData,font=('Arial',12,'bold')); SaveBtn.pack(side='right',padx=15,pady=10)
