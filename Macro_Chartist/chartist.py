...
def concat_datasets_to_axes(seriesDict: dict, DataStart: str = "2010-01-01", EndDateStr: str = datetime.date.today().strftime("%Y-%m-%d")):
    ...
    for _, value in seriesDict.items():
        ...
        if axes_name not in SeriesDict_agg:
            ...
            for ds in datalist:
                freq = Utilities.freqDetermination(ds)
                freq.DetermineSeries_Frequency()
                fr  = freq.frequency; pid = freq.per_in_d; cunt = freq.freq
...
def run_chartist(InputsPath: str = "", sheet_name: str = "", use_def_control: bool = False):
    ...
    if use_def_control: 
        InputsPath = wd+fdel+'Control.xlsm'
        sheet_name = "Input_Template"
    else:
        pass
    ...
    if InputsPath:
        Inputs = pd.read_excel(InputsPath, sheet_name=sheet_name, index_col=0, usecols="A:J", nrows=58)
    else:
        ...
        sheet_name = input("Enter the name of the sheet in the Excel file to read data from: ")
        Inputs = pd.read_excel(InputsPath, sheet_name=sheet_name, index_col=0, usecols="A:J", nrows=58)
