...
==============================
Downloads M2 money supply data and FX rates for multiple countries using
bootleg_datafeed (TradingView source), converts to USD, and constructs
aggregate Global M2 indexes.
"""
...
class Global_M2:
    ...
    ds : Dataset
        bootleg_datafeed Dataset instance for pulling data
    """
    ...
    def __init__(self, config_file='M2Info_Top50.xlsx', config_folder='UpdateM2Infos'):
        ...
        self.config_path = self._find_config_file(config_folder, config_file)
    ...
    def _find_config_file(self, config_folder: str, config_file: str) -> Path:
        """Find config file in package dir or repo root fallback locations."""
        candidates = [
            self.wd / config_folder / config_file,
            self.wd.parent.parent / 'Liquidity' / 'Global_M2' / config_folder / config_file,
        ]
        for p in candidates:
            if p.is_file():
                return p
        return self.wd / config_folder / config_file
    ...
    def download_data(self, n_bars=500, countries=None):
        ...
        for i, country in enumerate(countries_to_process):
            ...
            try:
                ...
                # Process and combine data — m2_data/fx_data are now Series (not DataFrames)
    ...
    def _process_country_data(self, country, m2_close, fx_data, currency_code, fx_symbol):
        ...
        country : str
            Country name
        m2_close : pd.Series
            M2 close price series
        fx_data : pd.Series or None
            FX close price series (None if USD)
        currency_code : str
        ...
        m2_close.index = pd.DatetimeIndex(m2_close.index).to_period('M').to_timestamp()
        ...
        if currency_code == 'USD':
            ...
            fx_close = fx_data.copy()
            fx_close.index = pd.DatetimeIndex(fx_close.index).to_period('M').to_timestamp()

            # Align FX data with M2 data
            fx_close = fx_close.reindex(m2_close.index).ffill()
    ...
    def create_aggregate(self, countries, name='Custom', use_ffill=True):
        ...
        if use_ffill:
            ...
            for country, series in country_series.items():
                ...
                series_aligned = series.reindex(common_index)
                series_filled = series_aligned.ffill()
                # Add to aggregate
    ...
    def clean_outliers(self, method='iqr', threshold=3.0, z_score_threshold=3.0, 
        ...
        for country in countries_to_process:
            ...
            if len(outlier_info['outlier_indices']) > 0:
                ...
                cleaned_series = cleaned_series.fillna(method='bfill').ffill()
