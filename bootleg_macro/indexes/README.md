...
```bash
# Install bootleg-macro (includes indexes by default)
pip install -e ./bootleg_macro
```

The indexes module is always installed when you install `bootleg-macro`.
...
```python
from bootleg_macro import Global_M2
...
Country configuration is stored in Excel files under `bootleg_macro/indexes/UpdateM2Infos/`:
| File | Contents |
|------|----------|
...
Each file uses the country name as the index with columns for `M2_Symbol`, `M2_exchange`, `FX_Symbol`, `FX_Exchange`, and `M2_currency_code`. Config files are resolved automatically — the handler searches the package directory.
